  ...
  const sections = useMemo(() => {
    ...
    return {
      header: {
        ...
        subtitle: `${booking.booking_items_summary || "1 session booked"} • ${booking.headcount || "—"} divers`,
        sites: booking.sites_summary || booking.site_name || "—",
        window:
          booking.first_session_at_local && booking.last_session_at_local
            ? `${formatIsoForDisplay(booking.first_session_at_local)} → ${formatIsoForDisplay(booking.last_session_at_local)}`
    ...
    <div style={styles.page}>
      ...
      {state.status === "ok" && booking && sections ? (
        <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 13, opacity: 0.85, marginTop: 4 }}>
                  {sections.header.sites}
                </div>

                <div style={{ fontSize: 12, opacity: 0.75, marginTop: 4 }}>
                  {sections.header.window}
                </div>
              </div>
          ...
          {/* Booking Scope */}
          <div style={styles.card}>
            <div style={styles.sectionTitle}>Booking Scope</div>
            <Row k="Sessions" v={booking.booking_items_summary || "—"} />
            <Row k="Divers" v={String(booking.total_pax ?? booking.headcount ?? "—")} />
            <Row
              k="Booking Window"
              v={
                booking.first_session_at_local && booking.last_session_at_local
            ...
            />
            <Row k="Sites" v={booking.sites_summary || booking.site_name || "—"} />
            <Row k="Primary Session ID" v={booking.session_id || "—"} />
            <Row k="Primary Itinerary ID" v={booking.itinerary_id || "—"} />
            <Row k="Primary Itinerary Title" v={booking.itinerary_title || "—"} />
          </div>
