/*
  ...
  Change Log:
    ...
    - 2026-02-15 - v1.2.0:
      - Keep button visible during refresh (no flash) + refresh() hook
    - 2026-02-15 - v1.3.0:
    ...
    - 2026-05-14 - v1.3.4:
      - Wire Operational Schedule rows to shared DashboardSessionCard presentation component.
      - Preserve existing Cancel/Delete behavior through card action slot.
*/
    ...
    return (
      <div style={styles.bannerWarn}>
        ...
        <div style={{ marginTop: 6, opacity: 0.85 }}>
          Fix for now: ensure exactly ONE active row in <code>aquorix.user_operator_affiliations</code>.
        </div>
      </div>
    );
  }
...
export default function SchedulePage() {
  ...
  async function handleCancel(session_id: number) {
    ...
    const ok = window.confirm(`Cancel session #${session_id}?`);
    if (!ok) return;

    setCancellingId(session_id);

    try {
      await cancelDashboardSession(session_id);
      refresh();
    } catch (err: any) {
                  ...
                  {map[date].map((s) => {
                    ...
                    return (
                      <DashboardSessionCard
                        ...
                        session={s}
                        variant="schedule"
                        priceLabel={formatMinorPrice(s.price_from_minor, s.currency)}
                        debugLabel={(s as any).unified_id || `#${s.session_id}`}
                        actions={actions}
                      />
                    );
                  })}
