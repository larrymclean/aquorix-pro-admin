/*
  ...
  Description: AQUORIX onboarding Step 4 – Final confirmation page. Two-panel layout, summary/feedback card, loader/redirect, and debug info. Fully consistent with Steps 1–3 for QA and user review.
  Author: Cascade AI (with Larry McLean)
  Created: 2025-09-06
  Last Updated: 2025-09-06
  Status: Production-ready, QA passed, checkpointed
  Dependencies: React
  Notes: Duplicates summary card below for feedback continuity. Loader/redirect on submit. Aligned with AQX Canonical Guide and onboarding FSD.
  Change Log:
...
const Step4Confirm: React.FC<Step4ConfirmProps> = ({ identity, userEmail, passwordSet, session, profileData, onBack, onSubmit, submitting }) => {
  return (
    <>
      <div style={{ display: 'flex', minHeight: 540, borderRadius: 12, overflow: 'hidden', background: '#fff', boxShadow: '0 2px 16px #0001', marginTop: 40 }}>
        ...
        <div style={{ flex: 1, background: '#fff', padding: '36px 32px', display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingTop: 32 }}>
          <div style={{ fontWeight: 700, fontSize: 28, color: '#2574d9', marginBottom: 8, letterSpacing: 0.5, textAlign: 'center' }}>Step 4: Confirm Your Details</div>
          <div style={{ color: '#222', fontSize: 16, marginBottom: 24, textAlign: 'center' }}>Review your information below. Make any changes if needed, then complete setup to enter your dashboard.</div>
          <div style={{ background: '#f5f9ff', borderRadius: '0.75rem', boxShadow: '0 1px 4px rgba(40,125,225,0.08)', padding: '1.25rem 1.5rem', margin: '0 auto 24px', width: '100%', maxWidth: 440, fontSize: '1rem', color: '#001c34' }}>
            ...
            <div style={{ marginBottom: 6 }}>Business Name: <span style={{ fontWeight: 500 }}>{profileData?.brandName || ''}</span></div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-start', marginTop: 32 }}>
            ...
            <button type="button" onClick={onSubmit} disabled={submitting} style={{ background: '#2574d9', color: '#fff', border: 'none', borderRadius: 6, padding: '12px 22px', fontWeight: 700, fontSize: 18, cursor: 'pointer', marginLeft: 16, opacity: submitting ? 0.7 : 1 }}>
      ...
      <div style={{ background: '#f5f9ff', borderRadius: '0.75rem', boxShadow: '0 1px 4px rgba(40,125,225,0.08)', padding: '1.25rem 1.5rem', margin: '32px auto 24px', width: '100%', maxWidth: 440, fontSize: '1rem', color: '#001c34' }}>
        <div style={{ fontWeight: 600, marginBottom: 8 }}>Step 4 of 4</div>
        <div style={{ marginBottom: 6 }}>First Name: <span style={{ fontWeight: 500 }}>{identity?.firstName || ''}</span></div>
        <div style={{ marginBottom: 6 }}>Last Name: <span style={{ fontWeight: 500 }}>{identity?.lastName || ''}</span></div>
        <div style={{ marginBottom: 6 }}>Email: <span style={{ fontWeight: 500 }}>{userEmail}</span></div>
