...
export default function App({ destinationTimeZone }: AppProps) {
  ...
  function buildPendingBookingRequest() {
    ...
    if (checkoutSessionQuote && checkoutSessionQuote.checkoutUrl) {
      window.location.href = checkoutSessionQuote.checkoutUrl
      return
    }
    ...
    setIsPreparingStripeCheckout(true)
    ...
    try {
      ...
      if (!checkoutResponse.ok || !checkoutData.ok) {
        ...
        window.alert(checkoutData && checkoutData.error ? checkoutData.error : "Checkout session creation failed.")
        return
      }
      ...
      setCheckoutSessionQuote({
        checkoutUrl,
        ledger: {
          currency: checkoutData.ledger.currency,
          amountMinor: checkoutData.ledger.amountMinor,
          amountDisplay: checkoutData.ledger.amountDisplay,
        },
        charge: {
          currency: checkoutData.charge.currency,
          amountMinor: checkoutData.charge.amountMinor,
          amountDisplay: checkoutData.charge.amountDisplay,
        },
        fx: {
          rate: checkoutData.fx.rate,
          source: checkoutData.fx.source,
        },
      })
    } catch (error) {
      ...
      setIsPreparingStripeCheckout(false)
    }
  }
          ...
          <div style={{ display: "grid", gap: 14 }}>
            ...
            >
              ...
              {checkoutSessionQuote && (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                    <span>Dive Total</span>
                    <span style={{ fontWeight: 700 }}>
                      {checkoutSessionQuote.ledger.amountDisplay} {checkoutSessionQuote.ledger.currency}
                    </span>
                  </div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                    ...
                    <span style={{ fontWeight: 800 }}>
                      ${checkoutSessionQuote.charge.amountDisplay} {checkoutSessionQuote.charge.currency}
                    </span>
                  </div>

                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, fontSize: 12, color: "#5f6b76" }}>
                    <span>Exchange Rate</span>
                    <span>
                      1 {checkoutSessionQuote.ledger.currency} = ${checkoutSessionQuote.fx.rate.toFixed(2)} {checkoutSessionQuote.charge.currency}
                    </span>
                  </div>

                  <div style={{ fontSize: 12, color: "#5f6b76" }}>
                    Your card will be charged in {checkoutSessionQuote.charge.currency} by AQUORIX. Your bank may apply additional foreign transaction fees.
                  </div>
                </div>
              )}
              ...
              <div style={{ marginTop: 16, display: "flex", justifyContent: "flex-end" }}>
                <button
                  ...
                  disabled={isPreparingStripeCheckout}
                  style={{
                    padding: "10px 14px",
                    borderRadius: 10,
                    ...
                    background: "#e6f4ec",
                    color: "#075c31",
                    cursor: isPreparingStripeCheckout ? "not-allowed" : "pointer",
                    fontWeight: 800,
                    whiteSpace: "nowrap",
                    opacity: isPreparingStripeCheckout ? 0.7 : 1,
                  }}
                >
                  {isPreparingStripeCheckout
                    ? "Preparing Payment..."
                    : checkoutSessionQuote
                      ? `Pay $${checkoutSessionQuote.charge.amountDisplay} ${checkoutSessionQuote.charge.currency} with Card`
                      : "Review Card Charge"}
                </button>
              </div>
            </div>
