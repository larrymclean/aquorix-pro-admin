/*
  ...
  Change Log (append-only):
    ...
    - 2026-03-12 - v1.3.0:
      - Require explicit week_start when calling canonical sessions API.
      - Align widget fetching with approved explicit-week rule.
      - Preserve parity grid adapter while removing implicit backend week selection.
*/
...
export async function fetchLegacySchedule(weekStart: string): Promise<ScheduleJson> {
  const params = new URLSearchParams({
    operator_slug: "blue-current-divers",
    week_start: weekStart,
  })

  const res = await fetch(
    `http://localhost:3001/api/v1/sessions?${params.toString()}`,
    { method: "GET" }
  )
  ...
  return {
    status: "success",
    message: `Canonical sessions adapted for parity widget (${weekStart})`,
    operator: {
