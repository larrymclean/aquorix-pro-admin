...
const itineraryTotal = itinerary
  ...
  const confirmedDiveSummary =
    availablePassengerCount > 0
      ? `${availablePassengerCount} confirmed ${availablePassengerCount === 1 ? "seat" : "seats"} across ${availableCount} ${availableCount === 1 ? "dive" : "dives"} • $${costPerDive.toFixed(2)} per diver per dive`
      : ""
  ...
  try {
    ...
    console.log("[AQUORIX] Backend response", data)
    window.alert(`Pending booking created: ${data.bookingId}`)
  } catch (error) {
        ...
        {checkoutPayload && (
          <div style={{ display: "grid", gap: 14 }}>
            ...
            >
              ...
              <div style={{ fontSize: 13, color: "#5f6b76", marginBottom: 4 }}>
                Operator Slug: {checkoutPayload.operatorSlug}
              </div>
              <div style={{ fontSize: 13, color: "#5f6b76", marginBottom: 4 }}>
                Timezone: {checkoutPayload.timezone}
              </div>
              <div style={{ fontSize: 13, color: "#5f6b76" }}>
                  ...
                  <tbody>
                    {sortedItinerary.map((item) => (
                      <tr key={item.id} style={{ background: item.kind === "waitlist" ? "#f7f7f7" : "transparent" }}>
                        ...
                        >
                          {item.kind === "available" ? `$${(45 * item.passengerCount).toFixed(2)}` : "WAITLIST"}
                        </td>
